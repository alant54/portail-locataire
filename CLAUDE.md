...
- `src/auth/current-tenant.ts`, `src/auth/current-user.ts`, `src/sync/index.ts` - cross-lane interfaces (`getCurrentTenant(lookup?): Promise<CurrentTenant | null>`, `getCurrentUser(lookup?): Promise<SessionUser | null>`, `runIncrementalSync(): Promise<SyncRunSummary>`); lanes replace bodies, never the returned shapes. **Both identity seams are async**: the session is a cookie and `cookies()` is awaited in this Next version, so a sync seam could never read one — `src/app/layout.tsx` awaits them and is itself `async`. The optional `SessionLookup` (`{ sessionId?, database? }`) lets a caller that already holds a session id skip the request scope, which is what keeps the contract test hermetic. `src/contracts.test.ts` fails if a lane narrows any of the three shapes. `getCurrentUser()` is the **role** seam: `CurrentTenant` has no role and a manager has no `tenant_ref`, so the management gate cannot be built on `getCurrentTenant()`. Its stub answers `manager`; `PORTAL_STUB_ROLE=tenant|anonymous` switches it for manual checks until lane B lands.
- `docs/RAPPORT.md` / `docs/RAPPORT.pdf` - the delivery report (regenerate the PDF with `python3 scripts/rapport-pdf.py`, needs `pip install reportlab markdown`). Demo accounts (password `portail2026`): `lea.martin@example.ch` (TEN-00005), `lucas.martin@example.ch` (co-tenant on the same lease), `adrien.clerc@example.ch` (TEN-00170), `gerance@example.ch` (manager).
...
- Ownership settled in `phase-0-hardening` — nobody edits a shared file: `(tenant)/layout.tsx` is lane B's session gate, `(admin)/layout.tsx` is lane C's **manager gate** (404, not a redirect), and the frozen nav exposes exactly one edit point, `#session-slot`, which lane B fills from its own component. The root nav is role-dependent: a manager gets **no** header links (the tenant layout redirects non-tenants to `/admin`, so tenant links would be dead; admin pages carry their own sub-nav), everyone else gets the tenant links and never a `/admin` link.
